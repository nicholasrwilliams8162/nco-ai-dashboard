...
router.get('/netsuite/status', (req, res) => {
  const row = db.prepare('SELECT account_id FROM netsuite_tokens WHERE id = 1').get();
...
router.post('/netsuite/initiate', (req, res) => {
  ...
  db.prepare(`
    INSERT INTO oauth_pending (state, verifier, account_id, client_id, client_secret)
    VALUES (?, ?, ?, ?, ?)
  `).run(state, verifier, accountId, clientId, clientSecret);
...
router.get('/netsuite/callback', async (req, res) => {
  ...
  try {
    ...
    db.prepare(`
      INSERT INTO netsuite_tokens (id, account_id, client_id, client_secret, access_token, refresh_token, expires_at, scope)
      VALUES (1, ?, ?, ?, ?, ?, ?, ?)
    ...
    `).run(oauthAccountId, oauthClientId, oauthClientSecret, access_token, refresh_token || null, expiresAt, scope || null);
...
router.post('/settings', (req, res) => {
  ...
  if (groqApiKey !== undefined) {
    if (!groqApiKey.trim()) {
      db.prepare("DELETE FROM app_settings WHERE key = 'groq_api_key'").run();
    } else {
      db.prepare("INSERT INTO app_settings (key, value) VALUES ('groq_api_key', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value")
        .run(groqApiKey.trim());
    }
  }
  res.json({ ok: true });
...
router.post('/netsuite/disconnect', (req, res) => {
  db.prepare('DELETE FROM netsuite_tokens').run();
  res.json({ ok: true });
});
